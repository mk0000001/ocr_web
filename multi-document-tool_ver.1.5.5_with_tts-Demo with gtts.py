#python -m venv {name}
version  = 'mdc_ver.1.5.5 with TTS Demo with gtts'
...
def tts_call():
    tts_call_thread = threading.Thread(target=tts, name="tts_call_thread")
    tts_call_thread.start()

def tts():
    tts_popup = tk.Tk()
    tts_popup.title("TTS - DEMO")
    
    progress_bar = ttk.Progressbar(tts_popup, length=100, mode='determinate')
    progress_bar.pack()

    folder_path = filedialog.askdirectory()

    src_files = glob.glob(os.path.join(folder_path, '*.txt'))
    ...
    for src_file in src_files:
       with open(src_file, 'r', encoding='utf-8') as f:
        ...
        tts.save(f"i{src_files.index(src_file) + 1}.mp3")

        pygame.mixer.init()
        pygame.mixer.music.load(f"i{src_files.index(src_file) + 1}.mp3")
        pygame.mixer.music.play()

        total_length = pygame.mixer.music.get_pos()

        # 음성 재생이 끝날 때까지 대기
        while pygame.mixer.music.get_busy() == True:
            current_position = pygame.mixer.music.get_pos() / 1000  # milliseconds to seconds
            progress_bar['value'] = (current_position / total_length) * 100
            root.update()
            continue
...
def document_popup():
    ...
    pdf_to_png_button = tk.Button(frame_top, text="TTS - DEMO", command=tts, width=15, height=2)
    pdf_to_png_button.pack(side="bottom", padx=(10, 10))
